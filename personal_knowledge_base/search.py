...
def shared_rank_candidates(
    keyword_candidates: list,
    vector_candidates: list,
    *,
    rrf_k: int | None = None,
) -> list[dict]:
    """Shared candidate fusion used by production and isolated evaluation indexes.

    Inputs may be IDs or dictionaries containing ``chunk_id``/``id``.  The
    original candidate fields are retained, while rank and RRF metadata are
    always generated by the same implementation as production search.
    """
    rrf_k = _bounded_int(rrf_k, settings.SEARCH_RRF_K, 1, 10000)

    def candidate_id(value):
        if isinstance(value, dict):
            return str(value.get("chunk_id") or value.get("id") or "")
        return str(value or "")

    keyword_by_id = {candidate_id(value): value for value in keyword_candidates if candidate_id(value)}
    vector_by_id = {candidate_id(value): value for value in vector_candidates if candidate_id(value)}
    fused = rrf_fuse(
        [candidate_id(value) for value in keyword_candidates if candidate_id(value)],
        [candidate_id(value) for value in vector_candidates if candidate_id(value)],
        rrf_k,
    )
    result = []
    for entry in fused:
        chunk_id = str(entry["chunk_id"])
        source = vector_by_id.get(chunk_id, keyword_by_id.get(chunk_id, {}))
        row = dict(source) if isinstance(source, dict) else {"id": chunk_id}
        row.update(entry)
        row.setdefault("id", chunk_id)
        result.append(row)
    return result
...
def hybrid_search_ex(
    ...
    *,
    ...
    if connection.vendor == "sqlite":
        keyword_ranked = _fts_ranked(tenant_id, kb_set, query, keyword_n) if keyword_n else []
        vector_ranked = _vector_recall(tenant_id, kb_set, query, vector_n, tenant, meta) if vector_n else []
    else:
        with ThreadPoolExecutor(max_workers=2) as search_pool:
            fts_future = search_pool.submit(_fts_ranked, tenant_id, kb_set, query, keyword_n) if keyword_n else None
            vec_future = search_pool.submit(_vector_recall, tenant_id, kb_set, query, vector_n, tenant, meta) if vector_n else None
    ...
    fused = shared_rank_candidates(keyword_ranked, vector_ranked, rrf_k=rrf_k)
